...
def preprocess_likert(evaluation):
    evaluation.columns = ['score']
    return evaluation.xs('likert dialogue', level='category').reset_index().pivot(index=('bot', 'item'), columns='label', values='score').sort_index()

def avg_cohensd(d1, d2, d3, d4, axis=None):
    d1d2 = cohensd(d1, d2)
    d1d3 = cohensd(d1, d3)
    d1d4 = cohensd(d1, d4)
    d2d3 = cohensd(d2, d3)
...
def cohensd(d1, d2, axis=None):
    ...
    s = np.sqrt(s1_s2__) # pooled standard deviation
...
grouped_barplot_cohensd(results_df, None, "", "", value_col="estimate",
                        filename="outputs/figures/effect_sizes_per_group_likert",
